...
void ungarbleVideo(char** imgList, int numImgs) 
{
  ...
  parallel_pipeline( /*max_number_of_live_token=*/4,       
		     make_filter<void,Mat>(
					      ...
					      [&](flow_control& fc)-> Mat{
						if( first<last ) {
						  return imread(*(first++),CV_LOAD_IMAGE_COLOR);
						} else {
						  fc.stop();
						  return imread(*(first),CV_LOAD_IMAGE_COLOR); // might cause file error
						}
					      }    
					      ) &
		     make_filter<Mat,Mat>(
					       ...
					       [](Mat frm_in){
						 Mat frm_out;
						 decreaseBrightnessFilter(frm_in, frm_out);
						 return frm_out;
					       } 
					  ) &
		     make_filter<Mat,Mat>(
					       ...
					       [](Mat frm_in){
						 Mat frm_out;
						 decreaseContrastFilter(frm_in, frm_out);
						 return frm_out;
					       } 
					  ) &
		     make_filter<Mat,Mat>(
					       ...
					       [](Mat frm_in){
						 Mat frm_out;
						 rearrangePixelsFilter(frm_in, frm_out);
						 return frm_out;
					       } 
					  ) &
		     make_filter<Mat,Mat>(
					       ...
					       [](Mat frm_in){
						 Mat frm_out;
						 rotateMatFilter(frm_in, frm_out);
						 return frm_out;
					       } 
					  ) &
