...
void encode(char* plainText, char* cypherText, xorKey* keyList, int ptextlen, int numKeys) {
  ...
  for(charLoop=0;charLoop<ptextlen;charLoop++) {
    ...
    //This is indeed running in parallel, it just sucks performance wise because we don't have enough keys, even with all of them.
    //This same code has been verified as improving performance for something like 100000000 keys or so in the file test.c, but
    //without that many keys there is just the small amount of overhead which slows it down.
    //Paralellization via a map&reogranization would undoubably be faster, but this is a demo of a reduction operator.
#pragma omp parallel for reduction(^:cipherChar) private(keyLoop) schedule(static,16)
    for(keyLoop=0;keyLoop<numKeys;keyLoop++) {

      cipherChar^=getBit(&(keyList[keyLoop]),charLoop);
       //       printf("Alien #%d says hello.\n",pthread_self());
    }
